...
export default function AddClassForm({ activeTab }: { activeTab: string }) {
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
      <h2 className="text-xl font-bold text-slate-800 mb-6">Tambah Kelas Baru</h2>
      {message && (
      ...
      <form id="addClassForm" action={handleAction} className="space-y-4">
        <div>
          <label className="block text-sm font-semibold text-slate-600 mb-2">Nama Kelas</label>
          <input type="text" name="name" required placeholder="Cth: Preschool 4" className="w-full text-slate-700 p-3 bg-slate-50 border border-slate-200 rounded-xl" />
        </div>
        <div>
